...
// Memoize footer content outside component to prevent recreation on every render
...
// Memoize WhatsApp messages outside component
...
export function SimpleFooter({ locale = 'es' }: SimpleFooterProps) {
  // Memoize content selection - only recalculates when locale changes
  const content = useMemo(() => FOOTER_CONTENT[locale], [locale])
  
  // Memoize WhatsApp message - only recalculates when locale changes
  const whatsappMessage = useMemo(() => 
    WHATSAPP_MESSAGES[locale] || WHATSAPP_MESSAGES.es, 
    [locale]
  )
