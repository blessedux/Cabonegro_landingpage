...
export default function NavbarEs() {
  ...
  const currentLocale = pathname.startsWith('/es') ? 'es' : pathname.startsWith('/zh') ? 'zh' : pathname.startsWith('/fr') ? 'fr' : pathname.startsWith('/en') ? 'en' : 'es'
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🔄 NavbarEs: Pathname changed, forcing navbar to show', { pathname })
    }
  }, [pathname])
  ...
  useEffect(() => {
    ...
    if (preloaderComplete) {
      const timer = setTimeout(() => {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('✅ NavbarEs: Preloader complete, showing navbar')
        }
  ...
  useEffect(() => {
    ...
    const checkInterval = setInterval(() => {
      ...
      if (shouldBeVisible && !isVisible) {
        ...
        setIsNavbarHidden(false)
        if (process.env.NODE_ENV === 'development') {
          console.log('⚠️ NavbarEs: Periodic check detected navbar not visible, forcing it to show')
        }
      }
  ...
  const prefetchLanguageRoute = (newLocale: string) => {
    ...
    // Aggressive prefetching - prefetch immediately and with high priority
    router.prefetch(targetPath)
    
    // Also prefetch the homepage route for faster switching
    if (pathWithoutLocale !== '') {
      router.prefetch(`/${newLocale}`)
    }
  }
