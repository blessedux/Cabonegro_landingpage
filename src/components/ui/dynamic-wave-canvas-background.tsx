...
const HeroWave = () => {
    ...
    const render = () => {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          for (let i = 0; i < 4; i++) {
            a += fastCos(i - d + time * 0.3 - a * u_x); // Slower (0.3 instead of 0.5)
            d += fastSin(i * u_y + a);
          }

          const wave = (fastSin(a) + fastCos(d)) * 0.5;
          const intensity = 0.25 + 0.3 * wave; // More visible but still subtle
          const baseVal = 0.1 + 0.15 * fastCos(u_x + u_y + time * 0.15); // More visible base
          const blueAccent = 0.15 * fastSin(a * 1.5 + time * 0.1); // More visible
          const purpleAccent = 0.12 * fastCos(d * 2 + time * 0.05); // More visible
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full opacity-50 z-0" />;
};
