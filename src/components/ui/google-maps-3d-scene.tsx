...
export default function GoogleMaps3DScene({
  ...
  const initializeMap = () => {
    ...
    if (!(window as unknown as { google?: any }).google.maps) {
      console.error('❌ window.google.maps is not available')
      return
    }
    
    if (!(window as unknown as { google?: any }).google.maps.MapTypeId) {
    ...
    try {
      ...
      const map = new (window as unknown as { google: any }).google.maps.Map(mapRef.current, {
        ...
        mapTypeId: mode === 'hybrid' ? (window as unknown as { google: any }).google.maps.MapTypeId?.HYBRID || 'hybrid' : 
                   mode === 'satellite' ? (window as unknown as { google: any }).google.maps.MapTypeId?.SATELLITE || 'satellite' :
                   (window as unknown as { google: any }).google.maps.MapTypeId?.TERRAIN || 'terrain',
